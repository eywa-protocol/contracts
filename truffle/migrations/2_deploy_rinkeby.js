...
module.exports = async (deployer, network, accounts) => {
  ...
  if (network === 'rinkeby') {
     try {
              ...
              exec(`${process.cwd()}/scripts/bash/update_env_adapter.sh 8081 NETWORK1 ${bridge.address} ${tupb.address}  ${proxyAdminBridge.address} ${env_file} ${nodeList.address}`, { maxBuffer: 1024 * 100000000 }, (err, stdout, stderr) => {
                if (err) {
