...
module.exports = async (deployer, network, accounts) => {
  ...
  if (network === 'rinkeby') {
     try {
              ...
              exec(`${process.cwd()}/scripts/bash/update_env_adapter.sh 8081 RINKEBY ${bridge.address} ${tupb.address}  ${proxyAdminBridge.address} ${env_file}`, { maxBuffer: 1024 * 100000000 }, (err, stdout, stderr) => {
                if (err) {
