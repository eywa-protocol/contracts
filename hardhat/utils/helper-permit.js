...
// async function signPermit(_owner, _spender, _value, _contract){
...
async function signPermit(sender, spender, val, token, nonce) {
  ...
  const EIP712Domain = [
   ...
   const tx = await web3Provider.send('eth_signTypedData_v4', [message.owner, data])
   const sig = await ethers.utils.splitSignature(tx)
