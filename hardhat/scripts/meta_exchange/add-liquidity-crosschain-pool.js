...
async function main() {
  ...
  if (network.name != "network2" && network.name != "harmonytestnet") {
    for (let i = 0; i < deployInfo[network.name].localToken.length; i++) {
      ...
      let chain2 = network.name.includes("network") ? new ethers.Wallet(process.env.PRIVATE_KEY_NETWORK2) : owner;
      // let chain2 = new ethers.Wallet(process.env.PRIVATE_KEY_NETWORK2);
      let to = network.name.includes("network") ? chain2.address : owner.address
      // let to = chain2.address;
      let from = owner.address;
      let hubChainName = network.name.includes("network") ? 'network2' : 'harmonytestnet';
  ...
  if (network.name == "network2" || network.name == "harmonytestnet") {
    ...
    for (let x = 0; x < deployInfo[network.name].crosschainPool.length; x++) {
      for (let i = 0; i < deployInfo[network.name].crosschainPool[x].coins.length; i++) {
        ...
        // console.log(deployInfo[network.name].crosschainPool[x].network)
        console.log(deployInfo[network.name].crosschainPool[x].network, await ERC20.attach(deployInfo[network.name].crosschainPool[x].coins[i]).balanceOf(owner.address));
      }
    }
