...
contract bn256g1_tests {
	...
	function testIsOnCurve() public view returns (bool) {
		...
		bn256g1.Point memory x = g.scalarMult(uint256(sha256("1")));
	...
	function testHashToPoint() public view returns (bool) {
		bn256g1.Point memory p = bn256g1.hashToPoint(sha256("hello world"));
	...
	function testEquality() public view returns (bool) {
		bn256g1.Point memory g = bn256g1.generator();
		bn256g1.Point memory a = g.scalarMult(9).pointAdd(g.scalarMult(5));
		bn256g1.Point memory b = g.scalarMult(12).pointAdd(g.scalarMult(2));
	...
	function testOrder() public view returns (bool) {
		bn256g1.Point memory z = bn256g1.scalarBaseMult(bn256g1.genOrder());
		require(z.isInfinity());

		bn256g1.Point memory one = bn256g1.scalarBaseMult(1);
		bn256g1.Point memory x = z.pointAdd(one);
