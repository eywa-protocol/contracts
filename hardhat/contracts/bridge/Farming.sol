...
contract FarmingNaive is IFarming, ReentrancyGuard {
    ...
    function getReward(address user) external view override returns (uint256) {
...
contract FarmingUnlockSpeed is IFarming, ReentrancyGuard {
    ...
    function _settleUnlockAmount(
        address staker,
        address token,
        uint256 lpStaked,
        uint256 upToBlockNumber
    ) internal view returns (uint256) {
