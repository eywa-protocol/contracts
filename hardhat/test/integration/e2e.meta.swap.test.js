...
contract('CurveProxy', () => {
    ...
    describe("end-to-end local test", () => {
        ...
        it("Uniswap Token Swap: network1 -> network2", async function () {

            oldBalance = await tokenToSwap.balanceOf(userNet2)
            ...
            const txIdMock = new Array(3).fill(addressToBytes32(deployInfo["network2"].bridge))
            ...
            // await routerA.synthBatchAddLiquidity3PoolMintEUSDSwapRequest(
            //     {
            //         token:tokensToSynth,
            //         amount:amounts,
            //         from:userNet1,
            ...
            await timeout(25000)
            assert(oldBalance.lt(await tokenToSwap.balanceOf(userNet2)))
        })

        it("Uniswap Token Swap: network3 -> network2", async function () {
            EUSD = await ERC20B.at(deployInfo["network2"].hubPool.lp)
            oldBalance = await tokenToSwap.balanceOf(userNet2)
            //synthesize params
            const synthParams = {
            ...
            const txIdMock = new Array(3).fill(addressToBytes32(deployInfo["network1"].curveProxyV2))
            ...
            // await routerC.synthBatchAddLiquidity3PoolMintEUSDSwapRequest(
            //     {
            //         token:tokensToSynth,
            //         amount:amounts,
            //         from:userNet3,
